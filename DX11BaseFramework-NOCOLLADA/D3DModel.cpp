	...
	if(fileIn)
	{
		while(fileIn)
		{
			...
			else if(checkString == L"mesh")
			{
				...
				{
					Vertex::Basic32 tempVert = subset.vertices[i];
					...
					for(int j = 0; j < tempVert.WeightCount; ++j)
					{
						...
						tempVert.Pos.x += (tempJoint.position.x + rotatedPoint.x) * tempWeight.bias;
						tempVert.Pos.y += (tempJoint.position.y + rotatedPoint.y) * tempWeight.bias;
						tempVert.Pos.z += (tempJoint.position.z + rotatedPoint.z) * tempWeight.bias;
					}
...
void D3DModel::LoadTexture(Texture* temp)
{
	texture = temp;
}
...
void D3DModel::Update(ID3D11DeviceContext* deviceContext, float dt, int animation)
{
	if(!staticMesh)
	{
		...
		for(int i = 0; i < numSubsets; i++)
		{
			...
			{
				Vertex::Basic32 tempVert = subsets[i].vertices[j];
				//clear out the position and normal before use
				tempVert.Pos = XMFLOAT3(0, 0, 0);
				tempVert.Normal = XMFLOAT3(0, 0, 0);
				...
				for(int k = 0; k < tempVert.WeightCount; ++k)
				{
					...
					//D3DXVec3Normalize(&tempVert.m_vNormal, &tempVert.m_vNormal);
				}
			...
			for(unsigned int j = 0; j < subsets[i].vertices.size(); j++)
			{
				subsets[i].vertices[j].Pos = subsets[i].positions[j];
			}
...
void D3DModel::RenderModel(ID3D11DeviceContext* deviceContext, ID3DX11EffectTechnique* activeTech, DX11Camera mCam)
{
	//Perform matrix operations for stored scale and position
	XMMATRIX mSphereRotation = XMMatrixRotationRollPitchYaw(mRotation.GetZ(), mRotation.GetY(), mRotation.GetX());
	XMMATRIX mSphereScale = XMMatrixScaling(mScale.GetX(), mScale.GetY(), mScale.GetZ());
	XMMATRIX mSpherePosition = XMMatrixTranslation(mPosition.GetX(), mPosition.GetY(), mPosition.GetZ());
	XMStoreFloat4x4(&mSphereWorld, XMMatrixMultiply(XMMatrixMultiply(mSphereRotation,mSphereScale), mSpherePosition));
	...
	unsigned int stride = sizeof(Vertex::Basic32);
	unsigned int offset = 0;

	for(int i = 0; i < GetNumSubsets(); ++i)
	{
		deviceContext->IASetVertexBuffers(0, 1, &GetSubsets()[i].vertexBuffer, &stride, &offset);
		deviceContext->IASetIndexBuffer(GetSubsets()[i].indexBuffer, DXGI_FORMAT_R32_UINT, 0);
