...
class DeferredParticles
{
	ID3DX11Effect* mFX;
public:
	DeferredParticles(ID3D11Device* device, const std::wstring& filename);
	~DeferredParticles();

	ID3DX11EffectScalarVariable* g_fTime;
	ID3DX11EffectVectorVariable* g_LightDir;
	ID3DX11EffectVectorVariable* g_vEyePt;
	ID3DX11EffectVectorVariable* g_vRight;
	ID3DX11EffectVectorVariable* g_vUp;
	ID3DX11EffectVectorVariable* g_vForward;

	ID3DX11EffectMatrixVariable* g_mWorldViewProjection;
	ID3DX11EffectMatrixVariable* g_mInvViewProj;
	ID3DX11EffectMatrixVariable* g_mWorld;

	ID3DX11EffectShaderResourceVariable* g_txParticleColor;
	ID3DX11EffectShaderResourceVariable* g_txMeshTexture;
	
	void SetWorldViewProj(CXMMATRIX M)                  { g_mWorldViewProjection->SetMatrix(reinterpret_cast<const float*>(&M)); }
	void SetWorld(CXMMATRIX M)                          { g_mWorld->SetMatrix(reinterpret_cast<const float*>(&M)); }
	void SetInvViewProj(CXMMATRIX M)              { g_mInvViewProj->SetMatrix(reinterpret_cast<const float*>(&M)); }
	void SetLightDirection(const XMFLOAT3& v)                  { g_LightDir->SetRawValue(&v, 0, sizeof(XMFLOAT3)); }
	void SetEyePos(const XMFLOAT3& v)                  { g_vEyePt->SetRawValue(&v, 0, sizeof(XMFLOAT3)); }
	void SetRight(const XMFLOAT3& v)                  { g_vRight->SetRawValue(&v, 0, sizeof(XMFLOAT3)); }
	void SetUp(const XMFLOAT3& v)                  { g_vUp->SetRawValue(&v, 0, sizeof(XMFLOAT3)); }
	void SetForward(const XMFLOAT3& v)                  { g_vForward->SetRawValue(&v, 0, sizeof(XMFLOAT3)); }
	void SetTime(float f)                           { g_fTime->SetFloat(f); }
	void SetParticleTexture(ID3D11ShaderResourceView* tex)   { g_txParticleColor->SetResource(tex); }
	void SetMeshTexture(ID3D11ShaderResourceView* tex)   { g_txMeshTexture->SetResource(tex); }
...
public:
	ID3DX11Effect* mFX;
	BillboardEffect(ID3D11Device* device, const std::wstring& filename, ID3DX11Effect*);
	~BillboardEffect();
	void SetWorldViewProj(CXMMATRIX M)                  { WorldViewProj->SetMatrix(reinterpret_cast<const float*>(&M)); }
	void SetWorld(CXMMATRIX M)                          { World->SetMatrix(reinterpret_cast<const float*>(&M)); }
	void SetWorldInvTranspose(CXMMATRIX M)              { WorldInvTranspose->SetMatrix(reinterpret_cast<const float*>(&M)); }
	...
	void SetFogColor(const FXMVECTOR v)                 { FogColor->SetFloatVector(reinterpret_cast<const float*>(&v)); }
	void SetFogStart(float f)                           { FogStart->SetFloat(f); }
	void SetFogColor(Vector3 v)                 { FogColor->SetFloatVector(reinterpret_cast<const float*>(&v)); }

	void SetRandomNumber(float f)                           { RandomNumber->SetFloat(f); }
	void SetFogRange(float f)                           { FogRange->SetFloat(f); }
	void SetDirLights(const DirectionalLight* lights)   { DirLights->SetRawValue(lights, 0, 3*sizeof(DirectionalLight)); }
	void SetMaterial(const Material& mat)               { Mat->SetRawValue(&mat, 0, sizeof(Material)); }
	void SetDiffuseMap(ID3D11ShaderResourceView* tex)   { DiffuseMap->SetResource(tex); }
	void SetNormalMap(ID3D11ShaderResourceView* tex)   { NormalMap->SetResource(tex); }
	void SetSSFlag(bool f)  { ssflag->SetBool(f); } 
	void SetToonFlag(bool f)  { toonflag->SetBool(f); } 
	void SetSobelFlag(bool f)  { sobelflag->SetBool(f); }
	void SetFogFlag(bool f)  { fogflag->SetBool(f); } 
	void SetGlitterFlag(bool f)  { GlitterFlag->SetBool(f); } 
	void SetGlassFlag(bool f)  { GlassFlag->SetBool(f); } 
	...
	ID3DX11EffectVariable* DirLights;
	ID3DX11EffectVariable* Mat;
	ID3DX11EffectScalarVariable *ssflag;
	ID3DX11EffectScalarVariable *sobelflag;
	ID3DX11EffectScalarVariable *toonflag;
	ID3DX11EffectScalarVariable *fogflag;
	ID3DX11EffectScalarVariable *GlitterFlag;
	ID3DX11EffectScalarVariable* RandomNumber;
	ID3DX11EffectScalarVariable *GlassFlag;
...
public:
	...
	static DeferredParticles* ParticleShader;
};
#pragma endregion
