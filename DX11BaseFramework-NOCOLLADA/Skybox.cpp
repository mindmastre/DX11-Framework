...
void Skybox::Draw(ID3D11DeviceContext* dc, const DX11Camera& camera)
{
	num += .00002f;
	// center Sky about eye in world space
	XMFLOAT3 eyePos = camera.GetPosition();
	XMMATRIX R = XMMatrixRotationRollPitchYaw(0, num, 0);
	XMMATRIX T = XMMatrixTranslation(eyePos.x, eyePos.y, eyePos.z);


	XMMATRIX WVP = XMMatrixMultiply(R * T, camera.ViewProj());
